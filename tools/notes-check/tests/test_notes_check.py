...
def test_N5_원본_노트에_있는_판_번호는_지난다():
    """원본이 `v6.08.8`처럼 자기 판 번호를 적는다. 우리 판 번호가 아니다.

    한 번에 여러 판을 얹으면 어디서 어디로 건너뛰는지가 사용자에게 쓸모 있는
    정보인데, 그것을 적는 순간 N5가 "이번 판이 아닌 판 번호"로 잡았다.
    """
    text = swap("### 준비물", "원본 v6.08.8을 얹었습니다.\n\n### 준비물")

    assert "N5" in codes(text)
    assert "N5" not in {v.code for v in notes_check.check(text, VERSION, "# v6.08.8\n\n원본 본문")}


def test_N5_원본_노트가_있어도_지난_우리_판은_걸린다():
    """면제는 원본이 이름을 댄 번호에만 준다.

    이것이 안 걸리면 N5가 잡으려던 것(지난 판을 복사해 한 자리를 안 고치는 것)이
    원본 노트를 받는 판마다 통째로 꺼진다.
    """
    text = swap("### v5.88.0.1 변경사항", "### v5.88.0.0 변경사항")

    assert "N5" in {v.code for v in notes_check.check(text, VERSION, "# v6.08.8\n\n원본 본문")}
...
def test_N19_옛_표지는_바꾼_판부터_걸린다():
    # 2026-09-26에 표지를 바꿨다. 옛 판 노트를 본으로 복사하면 옛 문구가 따라오는데,
    # 그 줄은 첫 항목보다 앞이라 N25의 목록 구간에도 안 들어와 아무 검사도 안 본다.
    새판 = notes_check.KO_PREFIX_SINCE
    옛것 = swap(CHANGES_BODY, OLD_KO_BODY).replace(VERSION, 새판)
    assert "N19" in codes(옛것, 새판)


def test_N19_옛_표지는_지난_판에서_통과한다():
    # 나가 있는 판은 소급해 고치지 않는다.
    assert notes_check.check(swap(CHANGES_BODY, OLD_KO_BODY), VERSION) == []


def test_N19_값을_적으면_걸린다():
    assert "N19" in codes(
        swap(CHANGES_BODY, "한국어 번역 문자열 수정: 있음.\n\n모드 변경사항: 없음.")
    )


def test_N19_아래에_목록이_없으면_걸린다():
    assert "N19" in codes(swap(CHANGES_BODY, "한국어 번역 문자열 수정:\n\n모드 변경사항: 없음."))
...
# **사용자가 `6.8.20.0`에서 셋을 고쳤다.** 발화 항목을 `와`/`과`로 이어 나열한
# 자리를 전부 쉼표로 바꿨다. 원문도 모드가 발화하는 문장도 쉼표인데 노트를 쓰면서
# `와` 연쇄로 지어낸 것이고, 스킬 132줄이 이미 "모드가 발화하는 고정 표현은
# `korean/strings.json`에서 확인하고 그대로 쓴다"고 정해 뒀는데 안 지켜졌다.
#
# **규칙이 문서에만 있으면 되돌아간다.** N24가 그 전례다 - `v5.93.0.2`의 판정이
# 문서 표에만 남아 `v5.94.0.0`에서 열두 자리가 되돌아갔다.

_CHANGE_LINE = "- 바탕화면 바로가기로 게임과 KR 달라무드 업데이터가 실행되도록 함."

#: 사용자가 고친 자리 셋. 왼쪽이 걸려야 하고 오른쪽이 통과해야 한다.
#: **둘 다 재지 않으면 아무것도 안 재는 규칙을 못 잡는다.**
_USER_FIXED_LISTS = (
    (
        "- 버디 창을 읽음. 초코보의 랭크와 경험치와 HP와 소환 남은 시간과"
        " 기술 포인트와 탭 이름을 음성 출력함.",
        "- 버디 창을 읽음. 초코보의 랭크, 경험치, HP, 소환 남은 시간,"
        " 기술 포인트, 탭 이름을 음성 출력함.",
    ),
    (
        "- 나에게 걸린 효과를 읽음. 효과 이름과 남은 시간과 설명을 음성 출력함.",
        "- 나에게 걸린 효과를 읽음. 효과 이름, 남은 시간, 설명을 음성 출력함.",
    ),
    (
        "- 제작 창을 읽음. 품질과 HQ 확률과 작업량과 내구도와 제작 공정을 음성 출력함.",
        "- 제작 창을 읽음. 품질, HQ 확률, 작업량, 내구도, 제작 공정을 음성 출력함.",
    ),
)


def test_N27_고치기_전_셋이_다_걸린다():
    for bad, _ in _USER_FIXED_LISTS:
        assert "N27" in codes(swap(_CHANGE_LINE, bad)), bad


def test_N27_고친_뒤_셋이_다_통과한다():
    for _, good in _USER_FIXED_LISTS:
        assert "N27" not in codes(swap(_CHANGE_LINE, good)), good


def test_N27_발화를_서술하지_않는_나열은_통과한다():
    """사용자가 `6.8.20.0`에서 **그대로 둔** 자리들이다.

    `처럼`으로 예시를 드는 자리, 설정 항목을 세는 자리, 그리고 이미 쉼표로 가른
    큰 나열 안에서 하위 묶음을 `과`로 묶어 층위를 세우는 자리다. 이 줄들이
    걸리면 규칙 쪽이 너무 넓다.
    """
    for line in (
        "- 낚시터와 던전과 돌발 임무처럼 개수를 알려 주는 안내에서 문장 끝"
        " 마침표가 빠져 다음 음성과 붙어 들리던 문제 수정.",
        "- 채팅 채널마다 윈도우 음성을 따로 고를 수 있음. 음성과 속도와 음량을"
        " 설정에서 정하며, 시스템에 음성이 없으면 스크린 리더가 읽음.",
        "- 원본이 더한 안내를 한국어로 옮김. 퀘스트 해금 안내, 제작 창과 제작수첩,"
        " 다른 플레이어를 지정했을 때의 직업과 레벨과 파티 소속임.",
    ):
        assert "N27" not in codes(swap(_CHANGE_LINE, line)), line


def test_N27_둘을_이은_것은_통과한다():
    # 셋부터 나열이다. 둘을 `와`로 잇는 것은 정상이고 `6.8.20.0`에 그대로 나간다.
    assert "N27" not in codes(
        swap(_CHANGE_LINE, "- 다른 플레이어를 지정하면 직업과 레벨을 같이 음성 출력함.")
    )


def test_N27_절을_넘어가서_세지_않는다():
    # 앞 문장이 발화를 말하고 뒤 문장이 다른 것을 나열하는 모양이다. `5.95.0.0`이
    # 실제로 그렇게 나가 있고 정당하다 - 나열의 서술어가 `나옴`이지 발화가 아니다.
    assert "N27" not in codes(
        swap(
            _CHANGE_LINE,
            "- 캐릭터 생성의 색상 이름 96개를 음성 출력함. 피부와 머리와 눈과 입술과"
            " 문신과 페이스 페인트의 색이 '따뜻한 갈색'처럼 나옴.",
        )
    )


def test_N27_변경사항_절만_본다():
    # 제한사항과 미검증 절은 습니다체 산문이라 `와` 연쇄가 정상이고, 발행본 여섯이
    # 다 갖고 있다. 범위를 넓히면 그날로 오탐이 된다.
    assert "N27" not in codes(
        swap(
            "- 방향 안내에 할당된 `N` 키가 게임의 제작 메뉴와 겹칩니다",
            "- 자동 이동과 길안내와 경로 미리 듣기가 고른 지점을 향하며 그것을 음성 출력합니다",
        )
    )


def test_N27_사용자가_고친_판은_안_걸린다():
