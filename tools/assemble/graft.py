...
자리는 원본 판마다 값이 바뀌는 곳이다. 고정 문자열로 적으면 다음 판에서 앵커가
통째로 깨진다.

지금 정규식을 쓰는 규칙은 없다. `<Version>`이 그런 자리였는데, 규칙은 데이터라 값을
바깥에서 못 받아서 개정 마디를 올릴 수가 없었다. 버전은 조립의 별도 단계가 찍는다
(`assemble.stamp_versions`).
