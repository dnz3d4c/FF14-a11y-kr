...
**같은 판끼리일 때만 성립한다.** 2026-09-09에 공식 `latest.zip`이 15.0.3.3으로 오른 뒤 설치본(아직 15.0.3.2)과 재니 **넷이 다 달라졌다.** 그런데 설치본을 15.0.3.3으로 올린 뒤 다시 재니 **넷이 다시 다 같았다.** 판 차이였을 뿐 한국 패치와는 무관하다. 대조는 반드시 **같은 어셈블리 판끼리** 해야 하고, 어긋난 채로 재면 "달라졌으니 표를 고쳐야 한다"는 잘못된 결론에 이른다.

**7.51 다운그레이드는 끝났다.** 한국판 업데이터 0.5.1부터 `FFXIVClientStructs`를 옛 판으로 갈아 끼우지 않고 공식이 싣는 판을 그 자리에서 패치한다. 그래서 이 저장소의 `graft` 덧대기 넷이 필요 없어졌다(`decisions.md`의 D-12 해제). 판정 근거는 설치본의 `Dalamud.KR.Compatibility.Patch.json`이고, 거기서 `OfficialClientStructsFileVersion`과 `CompatibleClientStructsFileVersion`이 같은 값이면 다운그레이드가 없다는 뜻이다.
