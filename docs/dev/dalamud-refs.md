...
뒤의 넷은 <https://goatcorp.github.io/dalamud-distrib/latest.zip>에서 받는다. **받은 것이 정말 같은지 sha256으로 확인한다** — 공식 버전이 올라가면 어느 날 달라질 수 있고, 그때 조용히 다른 어셈블리로 빌드하면 안 된다.
...
**어셈블리 버전이 오르면 릴리스를 새로 만든다.** 같은 태그에 자산을 갈아 끼우지 않는다 — 지난 버전으로 되돌려 빌드해 볼 수 없게 된다.
